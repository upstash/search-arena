...
export const useQueryState = <TValue extends string | undefined, TInitial extends TValue = TValue>(
  key: string,
  ...
  const setState: Dispatch<SetStateAction<TValue>> = useCallback(
    (dispatch) => {
      const previousState = new URLSearchParams(window.location.search).get(key) as TValue | null

      const newValue =
        dispatch instanceof Function ? dispatch((previousState ?? undefined) as TValue) : dispatch
  ...
  return [firstRun && state === undefined ? initialStateStored : state, setState] as [
    TValue,
